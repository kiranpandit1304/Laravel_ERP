...
function ResetDomainPage() {
    ...
    $("#filter_nature_of_business").val('');
    $("#filter_payment_terms").val('');
    $("#filter_name").val('');
    $(".is_have_gst").prop("checked", false);
...
function makeDataObject(data) {
    ...
    if ($("#filter_nature_of_business").val())
        data['nature_of_business'] = $("#filter_nature_of_business").val();
    else
        data['nature_of_business'] = "";

    if ($("#filter_payment_terms").val())
        data['payment_terms_days'] = $("#filter_payment_terms").val();
    else
        data['payment_terms_days'] = "";

    if ($('input[name="is_have_gst"]:checked').val())
        data['is_have_gst'] = $('input[name="is_have_gst"]:checked').val();
    else
        data['is_have_gst'] = "";

    if ($('.customer_top_dropdown').val())
        data['is_have_top_gst'] = $('.customer_top_dropdown').val();
    else
        data['is_have_top_gst'] = "";
...
$("#showCreateCusModal").on("click", function () {
    $(".step-tab").removeClass('active');
    $("#step-cc1").addClass('active');
    $(".customer_created").addClass("hide-d");
    $("#uid").val('')
    $(".save_customer").removeClass("hide-d")
    $(".next_btn").addClass("hide-d")
    $('#fname').val('');
...
$("body").on("submit", "#cusForm_d", function () {
    var bname = $("#fname").val();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status == true) {
                $("#uid").val(response?.client_id)
                $(".customer_created").removeClass("hide-d");
                 $(".updatebtn").removeClass("hide-d");

                // $("#CustomerDetailForm").removeClass("hide-d");
                $("#CustomerDetailForm").removeClass("hide-d");
                $(".save_customer").addClass("hide-d");
                $(".next_btn").removeClass("hide-d");
                var gst_no = $("#gst_no").val();
                $("#pan").val(gst_no.slice(2, -3));
                ResetDomainPage();
            } else {
...
$(".opencreatecustomer").on("click", function () {
...
$("a#close_cibsp").click(function (e) {
    // if (confirm("Are you sure?")) {
        ...
        $("#CustomerDetailForm").addClass("hide-d");
        $(".updatebtn").addClass("hide-d");
        $(".gst_field").prop("disabled", false);
        $(".business_name").prop("disabled", false);
   
});

$("body").on("click", ".show_detail_form", function () {
    $(".gst_field").prop("disabled", true);
    $(".business_name").prop("disabled", true);
    $(".customer_created").addClass("hide-d")
    $("#CustomerDetailForm").removeClass("hide-d");
    $(".updatebtn").removeClass("hide-d");
    $("#final_gst").val($("#gst_no").val());
});


// 06AAHCD4406B1Z8 get tax details
$("body").on("focusout", ".get_gst_data", function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status == true) {
                ...
                var bno = '';
                if(response?.pradr?.addr?.bno != undefined && response?.pradr?.addr?.bno != ''){
                    bno = response?.pradr?.addr?.bno 
                }
                ...
                $('.sv_billing_state').val(statelist[0]?.id).trigger('change');
...
$("body").on("click", ".address_copy", function () {
    ...
    if (typ == 'yes') {
        ...
        $(".sv_shipping_country").val($('.sv_billing_country').val()).trigger('change');
        $(".sv_shipping_state").val($('.sv_billing_state').val()).trigger('change');
        $("#shipping_zip_code").val($("#billing_zip").val())
        $(this).attr("data-id", "no");

    } else {
        $(".shi_add").val('');
        $(".sv_shipping_country").val('').trigger('change');;
        $(".sv_shipping_state").val('').trigger('change');;
        $("#shipping_zip_code").val('');
...
$("body").on("focusout", "#ifsc_code", function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('.bnk_state').val(statelist[0]?.id).trigger('change');;
                ...
                if (response?.media != null && response?.media != '' && response?.media != undefined && response?.media.length > 0) {
                    ...
                    for (var i = 0; i <= response?.media.length; i++) {
                        if (response?.media[i]?.customer_doc != '' && response?.media[i]?.customer_doc != undefined && response?.media[i]?.customer_doc != null) {
                            ...
                            mediaHtml += ' <div>';
                            mediaHtml += '       <span>';
                            mediaHtml += '        <img src="' + response?.media[i]?.customer_doc + '" alt=""> ';
                            mediaHtml += '     </span>';
                            mediaHtml += '    <a href=" ' + response?.media[i]?.customer_doc + '" target="_blank" > <h6>' + response?.media[i]?.customer_doc_name + '</h6> </a>';
                            mediaHtml += ' </div>';
