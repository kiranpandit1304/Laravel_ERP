...
class User extends Authenticatable
{
    ...
    public function purchaseReturnNumberFormat($number)
    {
        $settings = Utility::settings();

        return $settings["purchase_return_prefix"] . sprintf("%05d", $number);
    }

    public function saleNumberFormat($number)
    {
        $settings = Utility::settings();

        return $settings["sale_prefix"] . sprintf("%05d", $number);
    }
    public function transferNumberFormat($number)
    {
        $settings = Utility::settings();

        return $settings["transfer_prefix"] . sprintf("%05d", $number);
    }
    public function saleReturnNumberFormat($number)
    {
        $settings = Utility::settings();

        return $settings["sale_return_prefix"] . sprintf("%05d", $number);
    }
