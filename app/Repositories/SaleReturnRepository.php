...
class SaleReturnRepository extends BaseRepository
{
    ...
    public function storeSaleReturn($input):SaleReturn
    {
        try {
            ...
            $input['sale_return_items'] = json_decode($input['sale_return_items'],true);
            ...
            $replace = [
                $customer->name, $saleReturn->reference_code, $saleReturn->date, $saleReturn->grand_total,
...
//            $url = "https://xrjv8e.api.infobip.com/sms/2/text/advanced";
            ...
            if (isset($input['images']) && !empty($input['images'])) {
                foreach ($input['images'] as $image) {
                    $saleReturn['image_url'] = $saleReturn->addMedia($image)->toMediaCollection(SaleReturn::PATH,
...
//                }
    ...
    public function updateSaleReturnCalculation($input, $id)
    {
       ...
       /* if ($input['shipping'] > $input['grand_total'] || $input['shipping'] < 0) {
        ...
        $saleReturnInputArray['shipping_data'] = $input['shipping_data'];
        $saleReturnInputArray['tax_data'] = $input['tax_data'];
