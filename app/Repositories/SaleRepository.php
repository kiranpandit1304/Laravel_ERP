    ...
    public function storeSale($input): Sale
    {
        ...
        try {
          ...
          /*  echo "<pre>";
            ...
            if ($input['is_sale_created'] && $QuotationId && $QuotationId != 'undefined') {
            ...
            /*if($input['status'] == '2')
            {*/
                $input['sale_items'] = json_decode($input['sale_items'],true);
            ...
            $replace = [
                $customer->name, $sale->reference_code, $sale->date, $sale->grand_total, $payAmount, $dueAmount,
...
//            $url = "https://xrjv8e.api.infobip.com/sms/2/text/advanced";
            ...
            if (isset($input['images']) && !empty($input['images'])) {
                foreach ($input['images'] as $image) {
                    $sale['image_url'] = $sale->addMedia($image)->toMediaCollection(Sale::PATH,
    ...
    public function updateSaleCalculation($input, $id)
    {
        $sale = Sale::findOrFail($id);
        //$subTotalAmount = $sale->saleItems()->sum('sub_total');
        $subTotalAmount = SaleItem::where('sale_id',$id)
                                    ->select('sub_total')
                                    ->orderBy('sale_id','DESC')->first();
        $subTotalAmount = $subTotalAmount['sub_total'];                        
        //echo  $subTotalAmount; exit;
        ...
        $saleInputArray['shipping_data'] = $input['shipping_data'];
        $saleInputArray['tax_data'] = $input['tax_data'];
