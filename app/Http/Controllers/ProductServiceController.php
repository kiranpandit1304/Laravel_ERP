...
class ProductServiceController extends Controller
{
    ...
    public function create()
    {
        if (\Auth::user()->can('create product & service')) {
            ...
            $category     = ProductServiceCategory::where('parent_id', '=',  0)->where('type', '=', 0)->get()->pluck('name', 'id');
            $brands       = ProductBrand::get()->pluck('name', 'id');
            ...
            return view('productservice.create', compact('category', 'unit', 'tax', 'customFields', 'brands', 'baseUnits'));
        } else {
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('create product & service')) {
            ...
            $productService->base_unit_id        = $request->base_unit_id;
            $productService->unit_id        = $request->unit_id;
            $productService->sale_unit_id        = $request->sale_unit_id;
            $productService->stock_alert        = $request->stock_alert;
            $productService->quantity_limit        = $request->quantity_limit;
    ...
    public function edit($id)
    {
        ...
        if (\Auth::user()->can('edit product & service')) {
            if ($productService->created_by == \Auth::user()->creatorId()) {
                $category = ProductServiceCategory::where('parent_id', '=',  0)->where('type', '=', 0)->get()->pluck('name', 'id');
                $SingleSubCategoryEnity = ProductServiceCategory::where('id', '=', $productService->category_id)->where('type', '=', 0)->first();
                $SubCategories = ProductServiceCategory::where('parent_id', '=', $SingleSubCategoryEnity->parent_id)->where('type', '=', 0)->get()->pluck('name', 'id');
                $unit     = ProductServiceUnit::where('base_unit_id', '=', $productService->base_unit_id)->get()->pluck('name', 'id');
                ...
                return view('productservice.edit', compact('category', 'SubCategories', 'SingleSubCategoryEnity', 'unit', 'tax', 'productService', 'customFields', 'brands', 'baseUnits'));
            } else {
                return response()->json(['error' => __('Permission denied.')], 401);
            }
        } else {
    ...
    public function getSubCategories(Request $request)
    {
        $categories = [];
        if (!empty($request->id)) {
            $categories = ProductServiceCategory::where('parent_id', '=', $request->id)->get();
    ...
    public function getSubUnits(Request $request)
    {
        $unit = [];
        if (!empty($request->id)) {
            $unit = ProductServiceUnit::where('base_unit_id', $request->id)->get();
        }
    ...
    {
        $productServices = ProductService::where('id', $id)->first();
        /*$products = WarehouseProduct::where('product_id', '=', $id)->where('created_by', '=', \Auth::user()->creatorId())->get();*/
         $products = \App\Models\ManageStock::where('manage_stocks.product_id' , $id)
                                ->leftjoin('warehouses','manage_stocks.warehouse_id','warehouses.id')
                                ->select('manage_stocks.*','warehouses.name as warehouse')
                                ->get();
        return view('productservice.detail', compact('products', 'productServices'));
    }
