...
class ProductServiceCategoryController extends Controller
{
    public function index()
    {
        ...
        {
            $categories = DB::table('product_service_categories as ps')
            ->leftjoin('product_service_categories as b', 'ps.parent_id', '=', 'b.id' )
            ->select("ps.*", "b.name as parent_name")
            ->get();
    ...
    public function create()
    {
        ...
        {
            $allCategories = ProductServiceCategory::pluck('name', 'id');

            return view('productServiceCategory.create', compact('allCategories'));
        }
        else
        {
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create constant category'))
        {
            ...
            $category->parent_id  = $request->is_parent ? $request->parent_id : 0;
    ...
    public function edit($id)
    {
        ...
        if(\Auth::user()->can('edit constant category'))
        {
            ...
            $allCategories = ProductServiceCategory::where('id', "!=", $id)->where('parent_id', "!=", $id)->pluck('name', 'id');

            return view('productServiceCategory.edit', compact('category', 'allCategories', 'types'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
