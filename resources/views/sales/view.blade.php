...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{route('sale.index')}}">{{__('Sale')}}</a></li>
    <li class="breadcrumb-item">{{ Auth::user()->saleNumberFormat($sales->sale_id) }}</li>
@endsection
                        ...
                        <div class="card-body">
                            <div class="row timeline-wrapper">
                                <div class="col-md-6 col-lg-4 col-xl-4">
                                    ...
                                    <h6 class="text-primary my-3">{{__('Create Sale')}}</h6>
                                    <p class="text-muted text-sm mb-3"><i class="ti ti-clock mr-2"></i>{{__('Created on ')}}{{\Auth::user()->dateFormat($sales->sale_date)}}</p>
                                    @can('edit purchase')
                                        <a href="{{ route('sale.edit',\Crypt::encrypt($sales->id)) }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="ti ti-pencil mr-2"></i>{{__('Edit')}}</a>
                                ...
                                <div class="col-md-6 col-lg-4 col-xl-4">
                                    ...
                                    <h6 class="text-warning my-3">{{__('Send Sale')}}</h6>
                                    <p class="text-muted text-sm mb-3">
                                        @if($sales->status!=0)
                                            <i class="ti ti-clock mr-2"></i>{{__('Sent on')}} {{\Auth::user()->dateFormat($sales->sale_date)}}
                                        @else
                                            @can('send purchase')
                                    ...
                                    @if($sales->status==0)
                                        @can('send purchase')
                                            <a href="{{ route('sale.sent',$sales->id) }}" class="btn btn-sm btn-warning" data-bs-toggle="tooltip" data-original-title="{{__('Mark Sent')}}"><i class="ti ti-send mr-2"></i>{{__('Send')}}</a>
                                        @endcan
                                    @endif
                                </div>
                                <div class="col-md-6 col-lg-4 col-xl-4">
                                    ...
                                    @if($sales->status!= 0)
                                        @can('create payment purchase')
                                            <a href="#" data-url="{{ route('sale.payment',$sales->id) }}" data-ajax-popup="true" data-title="{{__('Add Payment')}}" class="btn btn-sm btn-info" data-original-title="{{__('Add Payment')}}"><i class="ti ti-report-money mr-2"></i>{{__('Add Payment')}}</a> <br>
                                        @endcan
                                    @endif
...
{{--                    @endif--}}
                    <div class="all-button-box mx-2">
                        <a href="{{ route('sale.resent',$sales->id) }}" class="btn btn-sm btn-primary">
                            {{__('Resend Sale')}}
                        </a>
                    </div>
                    <div class="all-button-box">
                        <a href="{{ route('sale.pdf', Crypt::encrypt($sales->id))}}" target="_blank" class="btn btn-sm btn-primary">
                            {{__('Download')}}
                        </a>
                    </div>
                </div>
                        ...
                        <div class="invoice-print">
                            <div class="row invoice-title mt-2">
                                ...
                                <div class="col-xs-12 col-sm-12 col-nd-6 col-lg-6 col-12 text-end">
                                    <h4 class="invoice-number">{{ Auth::user()->saleNumberFormat($sales->sale_id) }}</h4>
                                </div>
                                <div class="col-12">
                                    <hr>
                                </div>
                                    ...
                                    <div class="d-flex align-items-center justify-content-end">
                                        <div class="me-4">
                                            <small>
                                                ...
                                                {{\Auth::user()->dateFormat($sales->sale_date)}}<br><br>
                                            </small>
                                        </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col">
                                    <div class="float-end mt-3">
                                        ...
                                        {!! DNS2D::getBarcodeHTML(route('sale.link.copy',\Illuminate\Support\Facades\Crypt::encrypt($sales->id)), "QRCODE",2,2) !!}
                                    </div>
                                </div>
                            </div> 
                            <div class="row mt-3">
                                    ...
                                    <div class="table-responsive mt-3">
                                        ...
                                        <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td><b>{{$totalRate}}</b></td>
                                                <td></td>
                                                <td class="text-end">{{number_format($sales->getSubTotal(),2)}}</td>
                                            ...
                                            <tr>
                                                ...
                                                <tr>
                                                    ...
                                                    @php
                                                    $discount_in_pertage = ($sales->getSubTotal()/100) * @$sales->discount;
                                                    
                                                    @endphp
                                                    <td class="text-end">{{number_format($discount_in_pertage,2)}} ( {{  @$sales->discount }} ) %</td>
                                                </tr>
                                                @php
                                                $total_tax_charges=0;
                                                $tax_in_pertage=0;
                                                ...
                                                @php
                                                $grandTotal =0;
                                                $grandTotal = $sales->getSubTotal() + $total_tax_charges +$total_shipping_charges;
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-end">{{number_format(($grandTotal - $sales->getDue()),2)}}</td>
                                            </tr>
                                            <tr>
                                                <td colspan="6"></td>
                                                <td class="text-end"><b>{{__('Due')}}</b></td>
                                                <td class="text-end">{{number_format($sales->getDue(),2)}}</td>
                                            </tr>
                                            </tfoot>
                                        </table>
                                    </div>
