...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{route('purchase_return.index')}}">{{__('Purchase Return')}}</a></li>
    <li class="breadcrumb-item">{{ Auth::user()->purchaseReturnNumberFormat($purchase->purchase_return_id) }}</li>
@endsection
                        ...
                        <div class="card-body">
                            <div class="row timeline-wrapper">
                                <div class="col-md-6 col-lg-4 col-xl-4">
                                    ...
                                    <h6 class="text-primary my-3">{{__('Create Purchase Return')}}</h6>
                                    <p class="text-muted text-sm mb-3"><i class="ti ti-clock mr-2"></i>{{__('Created on ')}}{{\Auth::user()->dateFormat($purchase->purchase_date)}}</p>
                                    @can('edit purchase')
                                        <a href="{{ route('purchase_return.edit',\Crypt::encrypt($purchase->id)) }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" data-original-title="{{__('Edit')}}"><i class="ti ti-pencil mr-2"></i>{{__('Edit')}}</a>
                                ...
                                <div class="col-md-6 col-lg-4 col-xl-4">
                                    ...
                                    <h6 class="text-warning my-3">{{__('Send Purchase Return')}}</h6>
                                    <p class="text-muted text-sm mb-3">
                                        @if($purchase->status!=0)
                                            <i class="ti ti-clock mr-2"></i>{{__('Sent on')}} {{\Auth::user()->dateFormat($purchase->purchase_date)}}
                                        @else
                                            @can('send purchase')
                                    ...
                                    @if($purchase->status==0)
                                        @can('send purchase')
                                            <a href="{{ route('purchase_return.sent',$purchase->id) }}" class="btn btn-sm btn-warning" data-bs-toggle="tooltip" data-original-title="{{__('Mark Sent')}}"><i class="ti ti-send mr-2"></i>{{__('Send')}}</a>
                                        @endcan
                                    @endif
                                </div>
                                <div class="col-md-6 col-lg-4 col-xl-4">
                                    ...
                                    @if($purchase->status!= 0)
                                        @can('create payment purchase')
                                            <a href="#" data-url="{{ route('purchase_return.payment',$purchase->id) }}" data-ajax-popup="true" data-title="{{__('Add Payment')}}" class="btn btn-sm btn-info" data-original-title="{{__('Add Payment')}}"><i class="ti ti-report-money mr-2"></i>{{__('Add Payment')}}</a> <br>
                                        @endcan
                                    @endif
...
{{--                    @endif--}}
                    <div class="all-button-box mx-2">
                        <a href="{{ route('purchase_return.resent',$purchase->id) }}" class="btn btn-sm btn-primary">
                            {{__('Resend Purchase Return')}}
                        </a>
                    </div>
                    <div class="all-button-box">
                        <a href="{{ route('purchase_return.pdf', Crypt::encrypt($purchase->id))}}" target="_blank" class="btn btn-sm btn-primary">
                            {{__('Download')}}
                        </a>
                    </div>
                </div>
                        ...
                        <div class="invoice-print">
                            <div class="row invoice-title mt-2">
                                ...
                                <div class="col-xs-12 col-sm-12 col-nd-6 col-lg-6 col-12 text-end">
                                    <h4 class="invoice-number">{{ Auth::user()->purchaseReturnNumberFormat($purchase->purchase_return_id) }}</h4>
                                </div>
                                <div class="col-12">
                                    <hr>
                                </div>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <table class="table ">
                                            ...
                                            <tr>
                                                ...
                                                <td><b>{{$totalRate}}</b></td>
                                                <td></td>
                                                <td class="text-end">{{number_format($purchase->getSubTotal(),2)}}</td>
                                            ...
                                            <tr>
                                                ...
                                                <tr>
                                                    ...
                                                    @php
                                                    $discount_in_pertage = ($purchase->getSubTotal()/100) * @$purchase->discount;
                                                    
                                                    @endphp
                                                    <td class="text-end">{{number_format($discount_in_pertage,2)}} ( {{  @$purchase->discount }} ) %</td>
                                                </tr>
                                                @php
                                                $total_tax_charges=0;
                                                $tax_in_pertage=0;
                                                ...
                                                @if(!empty($TaxChargesDBValues) && count($TaxChargesDBValues))
                                                   ...
                                                   @php
                                                    $sub_total = $purchase->getSubTotal();
                                                ...
                                                @endif
                                                @php
                                                $grandTotal = $purchase->getTotal() + $total_tax_charges +$total_shipping_charges;
                                                @endphp
                                            <tr>
                                                <td colspan="6"></td>
                                                <td class="blue-text text-end"><b>{{__('Total')}}</b></td>
                                                <td class="blue-text text-end">{{number_format($purchase->getTotal(),2)}}</td>
                                            </tr>
                                            <tr>
                                                <td colspan="6"></td>
                                                <td class="text-end"><b>{{__('Paid')}}</b></td>
                                                <td class="text-end">{{number_format(($purchase->getTotal()-$purchase->getDue()),2)}}</td>
                                            </tr>

                                                <td colspan="6"></td>
                                                <td class="text-end"><b>{{__('Due')}}</b></td>
                                                <td class="text-end">{{number_format($purchase->getDue(),2)}}</td>
                                            </tr>
                                            </tfoot>
                                        </table>
                                    </div>
