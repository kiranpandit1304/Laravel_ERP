...
<div class="modal-body">
    <div class="row">
        ...
        <div class="form-group  col-md-6">
            ...
            {{ Form::select('category_id', $category, (!empty($SingleSubCategoryEnity->parent_id) ? $SingleSubCategoryEnity->parent_id : $productService->category_id), array('class' => 'form-control select category_d','required'=>'required')) }}
        </div>
        @php  $hide= empty($SingleSubCategoryEnity->parent_id) || $SingleSubCategoryEnity->parent_id==0 ? 'hide-d' : ''  @endphp 
        <div class="form-group col-md-6 show_sub_div {{ $hide }}">
            {{ Form::label('category_id', __('Sub Category'),['class'=>'form-label']) }}<span class="text-danger">*</span>
            {{ Form::select('category_id', $SubCategories, $SingleSubCategoryEnity->id, array('class' => 'form-control select show_sub_category')) }}
        </div>
        <div class="form-group col-md-6">
        ...
        <div class="form-group col-md-6">
            ...
            {{ Form::select('base_unit_id', $baseUnits,null, array('class' => 'form-control select base_unit_d','required'=>'required')) }}
        </div>

        <div class="form-group col-md-6">
            {{ Form::label('unit_id', __('Purchase Unit'),['class'=>'form-label']) }}<span class="text-danger">*</span>
            {{ Form::select('unit_id', $unit,null, array('class' => 'form-control select show_sub_unit','required'=>'required')) }}
        </div>

        <div class="form-group col-md-6">
            {{ Form::label('sale_unit_id', __('sale Unit'),['class'=>'form-label']) }}<span class="text-danger">*</span>
            {{ Form::select('sale_unit_id', $unit,null, array('class' => 'form-control select show_sub_unit','required'=>'required')) }}
        </div>
        <!-- <div class="form-group  col-md-6">
